...
class BaseSkill:
    ...
    def reset(self, config_update, info, params=None):
        self._params = params
        self._state = None
        self._config.update(config_update)
        if not self._config['ignore_aff_rew']:
    ...
    def get_gripper_ac(self, info):
        # params[rc_dim: rc_dim + rg_dim].copy()
        gripper_action = info['gripper']
    ...
    def _compute_aff_reward_and_success(self, info):
        ...
        if self._config['aff_type'] == 'dense':
            if aff_success:
                ...
                aff_reward = 1.0 - \
                    np.tanh(self._config['aff_tanh_scaling'] * min_dist)
        else:
...
class ReachOSCSkill(BaseSkill):
    ...
    def reset(self, config_update, info, params=None):
        ...
        self._num_reach_steps = 0
        super().reset(config_update, info)
    ...
    def is_success(self, info):
        ...
        if np.linalg.norm(pos - cur_pos) <= th:
            self._num_reach_steps += 1
        return (np.linalg.norm(pos - cur_pos) <= th and self._num_reach_steps >= self._config['num_reach_steps'])
...
class GraspSkill(BaseSkill):
    ...
    def update_state(self, info):
        ...
        if self._state == 'GRASPED' or \
                (self._state == 'REACHED' and is_grasp):
            self._state = 'GRASPED'
    ...
    def is_success(self, info):
        return self._num_grasp_steps >= self._config['num_grasp_steps'] or self._num_grasp_steps >= info.get('num_grasp_steps', 1000)
